...
class AdvisorProfile extends Model
{
    use HasFactory;
    protected $fillable = [
         'display_name','tagline','FCANumber','company_name','phone_number','address_line1','address_line2','city','postcode','web_address','facebook','twitter','about_us','role','image','short_description','status','advisorId','serve_range','linkedin_link','description','company_logo','network','email','gender','language','company_id','stripe_customer_id','FCA_verified','mortgage_min_size','mortgage_max_size','web_address_update','linkedin_update','fb_update','twitter_update','invalidate_fca'
    ];
    ...
    public static function getSearchedAdvisor($search){
        try {
            ...
            $data = $query->orderBy('id','DESC')->groupBy('id')->limit(20)->get();
            ...
            if (count($data)<20) {
                ...
                $data = $query->orderBy('id','DESC')->groupBy('id')->limit($limit)->get();       
